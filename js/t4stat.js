...
class Slot {
  ...
  onKeyPress(evt) {
    ...
    if (charCode === 38 || charCode === 81) {
      ...
      !exceptions.includes(charCode)
    ) {
      evt.returnValue = false;
      if (evt.preventDefault) evt.preventDefault();
    } else if (!exceptions.includes(charCode)) {
      // normal entry
      if (document.getElementById(`input${this.slot_num}`).value === "0")
        document.getElementById(`input${this.slot_num}`).value = "";
    }
  }
  ...
  onUpdate() {
    ...
    if (this.currentSteps !== this.futureSteps)
      document.getElementById(cost_id).innerHTML = this.getCostDisplay();
  ...
  applyColouration() {
    const input_id = `input${this.slot_num}`;
    if (!this.stat_name)
      return (document.getElementById(input_id).style.color = "blue");

    const allowed_max = this.getMaxStat(this.futureStat < 0);

    if (!document.getElementById(input_id).style)
  ...
  syncDisplayWithValues() {
    ...
    document.getElementById(slot_id).disabled =
      !!this.stat_name || this.stat.finished;
  ...
  confirm() {
    if (this.stat_name) {
      ...
      if (!document.getElementById(`slot${this.slot_num}`).style)
        document.getElementById(`slot${this.slot_num}`).style = {};
...
class Stat {
  ...
  updatePotentialSuccessDisplay() {
    document.getElementById(
      "potential_display"
    ).innerHTML = `Potential: ${this.future_pot} / ${this.pot}`;
    document.getElementById(
      "success_rate_display"
    ).innerHTML = `Success Rate: ${this.getSuccessRate()}%`;
  ...
  updateFormulaDisplay() {
    ...
    if (typeof this.finished === "number") {
      ...
      if (this.tec !== 255)
        display += ` <span style="color: red; font-size: 8px">(${this.tec} TEC)</span>`;
      display += `<br /><span style="color: blue; font-size: 10px">Mats: ${Object.keys(
        this.mats
      )
        .filter((mat) => this.mats[mat])
        .map((mat) => `${this.mats[mat]} ${mat}`)
        .join(" / ")} (Max: ${this.max_mats})</span>`;
      ...
      if (message)
        display += ` <span style="color: green; font-size: 8px">(${message})</span>`;
    }
    document.getElementById(
      "formula_display"
    ).innerHTML = `<span style="font-weight: bold; font-size: 12pt;">Steps</span><br /><br />${
      this.type === "w" ? "Weapon" : "Armor"
    } - Potential: ${this.starting_pot}<br />${display}`;
    document.getElementById("redo_button").disabled =
      !this.steps.redo_queue.length;
    document.getElementById("undo_button").disabled =
      !this.steps.formula.length;
    document.getElementById("repeat_button").disabled =
      !this.steps.formula.length;
  }
  ...
  getDisplay() {
    const display = this.condensed_formula.map(
      (step, index) =>
        ...
        } <span style="color: gray">(${step.pot_after}pot)</span>`
    );
...
class MainApp {
  ...
  loadSettings() {
    ...
    document.getElementById("mat_reduction").checked =
      data.mat_reduction || false;
  }
    ...
    for (const workspace_id in this.stats) {
      ...
      buffer.push(
        `<div style="display: inline-block; border: solid 1px blue; border-radius: 7px; padding: 3px;${
          focused ? " background-color: lightpink" : " background-color: none"
  ...
  promptRename() {
    let new_name = prompt(
      `What shall we name this current stat instance? Current name: ${this.current}`
    );

    if (!new_name) return;
    if (this.stats[new_name])
      return alert(
        "The name you selected is already in use by another stat instance."
      );
  ...
  spawn(id) {
    ...
    const mat_reduction =
      document.getElementById("mat_reduction").checked || false;
